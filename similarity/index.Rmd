...
Next, we computed a weighted [Jaccard index](https://en.wikipedia.org/wiki/Jaccard_index) for each compound pair. This number, between 0 and 1, measures the similarity between two side effect/indication profiles. 
...
Similarity networks were created using [Cytoscape](http://www.cytoscape.org/). For each of the three networks, modules of similar compounds were constructed using [MCL Clustering](http://micans.org/mcl/) from the [clusterMaker2 app](http://apps.cytoscape.org/apps/clusterMaker2). Structures, rendered from each compound's SMILES, were superimposed onto the nodes using the [chemViz2 app](http://apps.cytoscape.org/apps/chemviz2), although these diagrams are difficult to see in the video. The [CyAnimator app](http://apps.cytoscape.org/apps/cyanimator) illustrates the transition from substructure similarity modules to side effect similarity modules. The compounds from the largest structural similarity module are colored red. When compounds organized into side effect similarity modules, the red nodes cosegregated indicating concordance between side effect and structural similarity.
